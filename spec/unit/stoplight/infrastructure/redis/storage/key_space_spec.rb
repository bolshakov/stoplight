...
RSpec.describe Stoplight::Infrastructure::Redis::Storage::KeySpace do
  ...
  describe "#key" do
    ...
    context "with a pattern" do
      ...
      it { is_expected.to eq("stoplight:v5:system:{light}:*") }
    end

    context "with one piece" do
      let(:pieces) { ["metadata"] }

      it { is_expected.to eq("stoplight:v5:system:{light}:metadata") }
    end

    context "with multiple pieces" do
      let(:pieces) { ["this", "that"] }

      it { is_expected.to eq("stoplight:v5:system:{light}:this:that") }
    end
  end
end
