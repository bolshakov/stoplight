...
RSpec.describe Stoplight::Infrastructure::DataStore::Redis::UnboundedMetrics, :redis do
  subject(:unbounded_metrics) { described_class.new(script_manager:, redis:, light_name: SecureRandom.uuid) }

  let(:script_manager) { Stoplight::Infrastructure::DataStore::Redis::ScriptManager.new(redis:) }
  ...
  xdescribe "#consecutive_errors" do
