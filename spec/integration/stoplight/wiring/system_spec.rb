...
RSpec.describe Stoplight::Wiring::System do
  subject(:system) { described_class.new("system name", **system_defaults) }

  let(:system_defaults) { {} }

  describe "#new" do
    let(:system_defaults) do
      {
  ...
  describe "#light" do
    ...
    context "when the light with the same name exists in another system" do
      ...
      it "allows same name light in different systems" do
        another_system = described_class.new("another system")
    ...
    context "with skipped_errors" do
      ...
      let(:light) { system.light(name, skipped_errors:) }
      let(:skipped_errors) { [StandardError, KeyError] }

      it "sets skipped_errors" do
        is_expected.to have_attributes(skipped_errors:)
      end
    end
    ...
    context "with tracked_errors" do
      ...
      let(:light) { system.light(name, tracked_errors:) }
      let(:tracked_errors) { [StandardError, KeyError] }

      it "sets tracked_errors" do
        is_expected.to have_attributes(tracked_errors:)
      end
    end
